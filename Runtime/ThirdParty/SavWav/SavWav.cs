...
namespace Virbe.Core.ThirdParty.SavWav
{
...
//
    public static class SavWav
    {
        ...
        public static byte[] GetWav(AudioClip clip, out uint length, bool trim = false)
        {
            ...
            WriteHeader(data, hz, channels, length, samples);

            return data;
        }

        public static byte[] GetWavF(float[] samples, uint frequency, ushort channels, out uint length,
            bool trim = false)
        {
            var data = ConvertAndWriteF(samples, out length, out var samplesT, trim);
            
            WriteHeader(data, frequency, channels, length, samplesT);
        ...
        private static byte[] ConvertAndWrite(AudioClip clip, out uint length, out uint samplesAfterTrimming, bool trim)
        {
            ...
            return ConvertAndWriteF(samples, out length, out samplesAfterTrimming, trim);
        }

        private static byte[] ConvertAndWriteF(float[] samples, out uint length, out uint samplesAfterTrimming,
            bool trim)
        {
        ...
        private static void WriteHeader(byte[] stream, uint hz, ushort channels, uint length, uint samples)
        {
            ...
            // NOTE: We commented this our to get rid of extra warning in Unity logs ("CS0219: The variable 'two' is assigned but its value is never used").
            // const ushort two = 2;
            const ushort one = 1;
