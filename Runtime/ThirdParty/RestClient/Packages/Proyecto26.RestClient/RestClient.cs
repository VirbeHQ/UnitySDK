...
namespace Proyecto26
{
    ...
    public static partial class RestClient
    {
        ...
        {
            get { return typeof(RestClient).Assembly.GetName().Version; }
        }
        ...
        {
            options.Method = UnityWebRequest.kHttpVerbPOST;
            if (Debug.isDebugBuild) options.EnableDebug = true;
            Request(options, callback);
        }
        ...
        /// <param name="callback">A callback function that is executed when the request is finished.</param>
        /// <typeparam name="T">The element type of the array.</typeparam>
        public static void PostArray<T>(string url, string bodyString,
            Action<RequestException, ResponseHelper, T[]> callback)
        {
